...
const App = {
  ...
  templates: {
    ...
    dynv6_subdomain: {
      ...
      webhook_url:
        "https://dynv6.com/api/v2/zones/YOUR_ZONE_ID/records/YOUR_RECORD_ID",
      webhook_headers: {
        Authorization: "Bearer YOUR_TOKEN",
        "Content-Type": "application/json",
      },
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.openTaskModal(-1));
    document
    ...
    document
      ...
      .addEventListener("click", () => this.updateTaskFromModal());
    document
      .getElementById("modal-delete-btn")
      .addEventListener("click", () => this.deleteTask());
    document
      .getElementById("modal-test-btn")
      .addEventListener("click", () => this.testRunTask());
    document
      .getElementById("modal-copy-btn")
      .addEventListener("click", () => this.copyTask());
  ...
  async fetchConfig() {
    try {
      ...
      this.elements.settingRunOnStartup.checked =
        !!this.state.config.run_on_startup;

      // Settings change listeners
      this.elements.settingLogLimit.addEventListener("change", () =>
        this.markDirty(),
      );
      this.elements.settingRunOnStartup.addEventListener("change", () =>
        this.markDirty(),
      );
  ...
  renderTasks() {
    ...
    tasks.forEach((task, index) => {
      ...
      // Name
      clone.querySelector(".task-name").textContent =
  ...
  openTaskModal(index) {
    ...
    this.elements.modalTitle.textContent = isNew
      ? "Add New Task"
      : "Edit Task Details";
    document.getElementById("modal-delete-btn").style.display = isNew
      ? "none"
      : "block";
    document.getElementById("modal-copy-btn").style.display = isNew
      ? "none"
      : "block";
    document.getElementById("modal-save-btn").textContent = isNew
      ? "Add Task"
      : "Done";
  ...
  updateTaskFromModal() {
    ...
    const existingTask =
      this.state.currentTaskIndex !== -1
        ? this.state.config.tasks[this.state.currentTaskIndex]
        : null;
  ...
  async saveConfig() {
    ...
    this.state.config.log_limit = Math.max(
      1,
      parseInt(this.elements.settingLogLimit.value) || 100,
    );
    this.state.config.run_on_startup =
      this.elements.settingRunOnStartup.checked;
  ...
  appendLog(logData) {
    ...
    this.elements.logsOutput.insertBefore(
      div,
      this.elements.logsOutput.firstChild,
    );
